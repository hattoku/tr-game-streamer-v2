...
UI仕様書 v1.9→v1.10、トークン仕様書 v1.5。build 成功（未コミット）。[[2026-09-11-layout-and-system-pages]] に追記。
...
build 成功（未コミット）。[[2026-09-11-layout-and-system-pages]] に追記。
...
`MobileNavBar.tsx` を新設、`MobileDrawer.tsx` を削除。build 成功（未コミット）。
...
エラーページ仕様書の「トレヨミ」誤記を修正（v1.1）。build 成功、`next start` で 200/404 を curl 確認（未コミット）。
...
tsc / next build 成功、ビルド後CSSにトークン・ユーティリティの生成を確認（未コミット）。
...
HANDOFF.mdのフェーズ2.5セクションに反映（未コミット）。
