...
| チャンネルから探す | `/channels` | ~~準備中ページ~~ → **2026-09-22 本実装済み**（フェーズ6ステップ1） |
| チャンネル詳細 | `/channels/[channelId]` | ~~未実装~~ → **2026-09-22 本実装済み**（同上。「流入元チャンネル一致判定」も実装） |
...
管理画面・法務ページ。「実際に使って必要だと感じた順」で入れるほうが無駄が少ない。

**2026-09-20追記（計画見直し）**: 楽天ブックスAPI連携（パッケージ画像取得）は当初フェーズ6以降の
予定だったが、フェーズ4.5完了後に`/games`一覧で実データを確認したところ、テストデータ5件すべてが
サムネイル無しのプレースホルダー表示だった。フェーズ5（自己ユーザーテスト期間）は「実際に使う」
検証であり、パッケージ画像が無い状態で使い込んでも得られる気づきの質が落ちるため、フェーズ5開始前の
ステップ8として前倒しする。
...
2. ~~**ゲームタイトル追加UI（管理者）**~~ → **2026-09-20対応済み**。
   ...
   - `components/games/GameCreateModal.tsx` — `GameSelectModal`のフッター（`onAddNew`prop、渡された
     ...
     設定変更不要）。楽天ブックスAPI連携はフェーズ4.5ステップ8（2026-09-20計画見直しで前倒し、
     旧予定はフェーズ6以降）。登録成功時は作成したゲームをその場で
...
6. ~~**`/settings`最小版**~~ → **2026-09-20対応済み**。
   ...
   実装は`app/(main)/settings/page.tsx`。各項目とも`users/{uid}`の対応フィールドを直接読み書きする
   （動画プレーヤーの連続再生・レビュー一覧のネタバレ非表示と同じフィールドを共有するため、設定ページで
   変更すると両方に反映される）。連続再生・ネタバレ非表示・マイリスト新着通知は`Checkbox`で即時保存
   （このアプリのトグルは既存のパターンに合わせてチェックボックスで統一、専用のスイッチ部品は追加していない）、
   表示名は入力＋「保存する」ボタンで確定（プロフィール仕様書§9.4のバリデーション＝1〜30文字・空欄不可を流用）。
   ログアウトは確認ダイアログなしで即時実行（§7.2）。未ログインは`/login`へリダイレクト。
   - **調査で判明**: ユーザードロップダウン（`UserDropdown.tsx`）は本ステップ着手前から既に
     「設定」リンクを`/settings`へ向けて実装済みだった（ページ自体が無く404になっていた）ため、
     導線側の追加作業は不要だった。
   - **副次的に修正したバグ**: `AuthContext`のヘッダー表示名取得が`getDoc`による一度きりの取得
     だったため、設定ページで表示名を変更してもヘッダーのユーザーメニューが画面遷移・再読み込み
     までは古い名前のまま表示され続けていた。`onSnapshot`による購読に変更し、他画面
     （`NotificationBell`の未読件数等）と同じ「ユーザー固有データはonSnapshotで購読する」方針に揃えて解消。
   - `npm run lint` / `npx tsc --noEmit` / `npm run build`成功。**動作確認**（本番Firestore接続、
     `npm run dev`のテストモードでPlaywright使用）: 管理者・一般ユーザーそれぞれでログイン→
     表示名の空欄保存エラー表示→正常値での保存とヘッダーへの即時反映（リロードなし）→
     連続再生トグルの変更が再読み込み後も保持されることをFirestoreへの実書き込みで確認→
     ログアウトで`/login`へ遷移、未ログインで`/settings`に直接アクセスすると`/login`へリダイレクト
     されることを確認。767px相当のモバイル幅でも表示崩れが無いことを確認。767/768/1600pxでの
...
8. ~~**楽天ブックスAPI連携（パッケージ画像取得）**~~ → **2026-09-20対応済み（ブラウザE2E確認済み）**。
   - 楽天ウェブサービスのアプリID・アクセスキーを新規取得（`.env.local`に`RAKUTEN_APPLICATION_ID`・
     `RAKUTEN_ACCESS_KEY`として設定。`YOUTUBE_API_KEY`と同じ扱いで`lib/constants.ts`・
     `SECRET_MANAGEMENT.md`・`deploy.ps1`/`deploy.sh`にも追加）。API Access Scopesは
     「Rakuten Books API」のみ選択。
   - `lib/rakuten.ts`（新規）— 楽天ブックスゲーム検索API（`BooksGame/Search`, version:2017-04-04）
     をfetchで直接呼び出す`searchRakutenGames(title)`。`app/api/admin/games/rakuten-search`
     （新規、`requireAdmin`）経由でのみ呼ばれ、キーはブラウザに露出しない。
   - **公式ドキュメント記載と実際のAPI挙動が食い違っていた点**（2026-09-20、実キーでの直接検証で判明。
     ドキュメント通りに実装すると全件0件or 403になっていた）:
     - 2026-02の楽天API基盤刷新以降、リクエストに`Referer`・`Origin`ヘッダー（アプリ登録時の
       「Allowed websites」と一致する値）が無いと`403 REQUEST_CONTEXT_BODY_HTTP_REFERRER_MISSING`で
       拒否される。ドキュメントに明記が無かった。サーバー間通信のため`https://puremite.net`固定で送信
       （`lib/rakuten.ts`の`REQUEST_ORIGIN`）。
     - `formatVersion=2`のレスポンスは`items`ではなく`Items`（先頭大文字）で返る。
     - `jan`フィールドはドキュメント上`long`だが実際は文字列で返る。
   - `components/games/GameCreateModal.tsx` — パッケージ画像欄をURL手入力からタイトル検索→候補選択
     （サムネイル・タイトル・対応機種/発売日つきの候補リスト）に変更。デフォルトは検索モードだが、
     ヒットしない場合（洋ゲー・マイナータイトル等）のため「URLを直接入力する」で従来のURL手入力にも
     切替可能。ゲームタイトル名の入力に検索キーワードを自動追従（未編集の間のみ）。
   - **動作確認**: `npm run lint` / `npx tsc --noEmit` / `npm run build`成功。楽天API自体は実キーで
     直接呼び出し、実データ（スプラトゥーン3で24件ヒット、画像URL・発売元・JANコード等を含む
     正常なレスポンス）を確認済み。`/api/admin/games/rakuten-search`も無認証アクセスで401
     （`requireAdmin`が機能）を確認済み。**ブラウザ経由のE2E**（stg、Playwright、管理者ログイン）:
     `/playlists/new`→ゲームタイトル選択→新規登録→「星のカービィ ディスカバリー」でタイトル名が
     検索欄に自動反映→検索実行→8件の候補（サムネイル・対応機種・発売日つき）表示→選択→
     プレビュー表示→登録→その場で自動選択され`/playlists/new`に反映→`/games`一覧でパッケージ画像が
     正しく表示されることを確認。確認に使ったテストデータ（ゲームタイトル1件、genre/theme未選択の
     ため集計カウント加算なし）はAdmin SDKで削除済み、stg DBへの影響は残っていない。
   - **副産物のバグ発見・解決**: 動作確認の過程で、stg環境のテストモード「管理者でログイン」
     「一般ユーザーでログイン」が両方とも`INVALID_LOGIN_CREDENTIALS`で失敗する事象に遭遇
     （`/api/test/sign-in`は200で新パスワードを返すが、直後の`signInWithPassword`が拒否される）。
     調査の結果、**原因はコードのバグではなく、2026-09-20 19:01から起動しっぱなしだった
     `next dev`プロセス（PID 6828）が、それより前のセッションで設定された環境変数を引き継いだまま
     残っていたこと**と判明（`adminAuth`（サーバー）と`auth`（ブラウザ）が異なるFirebase
     プロジェクトを見ていたため、パスワードは正しく設定されるが照合先が食い違っていた）。
     該当プロセスを終了し`npm run dev`を再起動したところ解消。**教訓**: `npm run dev`系の
     プロセスを長時間（セッションをまたいで）起動したままにせず、接続先が疑わしい動作を見たら
     まずプロセスの再起動を疑う。
   - **既存テストデータ5件（`scripts/data/test-games.mjs`由来）へのバックフィル** →
     **2026-09-21完了、stg 5件全件にパッケージ画像設定済み**。
     `scripts/backfill-package-images.mjs`（新規）で既存ゲームのタイトル名から楽天ブックスを検索し
     直接Firestoreを更新する（`npm run backfill:package-images:stg` / `:prod`、既定はdry-run、
     `--apply`で書き込み）。`--apply`でのFirestore書き込みはauto modeの許可分類器に
     「Modify Shared Resources」としてブロックされることがあるため、ユーザーが自身のターミナルで
     実行した。
     - タイトル名検索で自動マッチしたのは5件中2件（スプラトゥーン3、ポケットモンスター
       スカーレット・バイオレット）のみ。
     - 残り3件（ゼルダの伝説 ティアーズ オブ ザ キングダム・モンスターハンターライズ・
       エルデンリング）はタイトル名検索では0件ヒットだった。**原因判明**: 対象商品の
       `availability`（在庫状況）フィールドが空文字になっており、既定の`outOfStockFlag=0`
       （品切れ等除外）で弾かれていた。ドキュメント上`availability`は0〜6の整数のみのはずだが、
       実際には空文字の商品が存在する。パッケージ画像取得が目的で購入可否は問わないため、
       `outOfStockFlag=1`を`lib/rakuten.ts`（本番の検索モーダル）・
       `scripts/backfill-package-images.mjs`双方の既定パラメータに追加して恒久対応した。
     - エルデンリングは`platforms`が`["PS5","Steam"]`なのに実商品は`hardware:"PS4"`（PS4版）
       だったため、機種名の自動照合ロジックでは依然マッチしない。ユーザーが楽天ブックスサイトで
       目視確認したJANコード3件（ティアーズ オブ ザ キングダム/モンスターハンターライズ/
       エルデンリング）を`scripts/backfill-package-images.mjs`の`MANUAL_JAN_OVERRIDES`に
       手動登録し、JAN直接検索（機種名照合をスキップ）で反映した。

これでフェーズ4.5（ドッグフーディング準備）の全8ステップが完了。次はフェーズ5（自己ユーザーテスト期間）。

**見積**: 8セッション前後（ステップ1・2・4が重め）。
...
3. **2026-09-22 ゲームタイトル登録: フォームを再構成（商品ページURLを先頭に・テーマ/機種をプルダウンに）**
   （フェーズ5のユーザー指摘: テーマのチップ50件超が画面の大半を占める、楽天URL欄が「URLを直接入力する」を
   押さないと出てこない、機種候補が5件しかない）。
   - `components/games/GameCreateModal.tsx` — 入力順を「楽天ブックス商品ページURL（先頭・autoFocus、
     取得結果のプレビューを直下に表示）→ タイトル名 → ジャンル → テーマ → プラットフォーム →
     パッケージ画像（楽天タイトル検索＋画像URL直接入力を常時表示）→ 説明文」に変更。
     検索モード／URL手入力モードの切り替え（`imageMode`）は廃止。編集モードでも楽天URL・画像URLの
     初期値が各欄にそのまま見える。
   - テーマ・プラットフォームは新設の共通UI `MultiSelectMenu`（`components/ui/DropdownMenu.tsx`、
     Radix `CheckboxItem`ベースで選んでも閉じない・最大高さ内でスクロール・表示部は選択件数・先頭に部分一致の
     絞り込み検索窓（開くとフォーカス、閉じると消える。ジャンル/テーマの候補は「その他」を末尾に））＋
     選択済みだけをチップ表示（クリックで解除）。デザイントークン仕様書 §10 に複数選択セレクトの表現を追記。
   - `PLATFORM_OPTIONS`を楽天ブックスの機種一覧＋Steamの18機種に拡充（Nintendo Switch / Nintendo Switch 2 /
     Wii U / Wii / ニンテンドー3DS / ニンテンドーDS / PS5 / PS4 / PS3 / PS2 / PS Vita / PSP / Xbox Series X /
     Xbox One / Xbox 360 / Steam / おもちゃ / その他。ユーザー決定: Steamは残す、UIはプルダウン）。表記は楽天APIの
     `hardware`値に合わせてあるので、旧機種でも自動セットが効く。`games.platforms`のサーバー側検証は従来どおり無し。
   - 仕様書: 管理 マスタ管理仕様書 §7.1（v2.6）・ページ 再生リストを追加する 仕様書 §4.2.1（v3.9）・
     共通 デザイントークン仕様書 §10（v2.1）。
   - **動作確認**: `npm run lint` / `npx tsc --noEmit`成功。ローカル`npm run dev`（stg）＋Playwrightで、
     タイトル検索→候補選択と、商品ページURL（計測パラメータ付き）貼り付け→Enterの両方で自動セット
     （タイトル名・ジャンル・テーマ「アドベンチャー（一般）」・機種「Nintendo Switch 2」・画像URL）を確認。
     テーマのプルダウンで連続選択しても閉じないこと、チップ解除、375px幅ではみ出さないこと、
     ゲームタイトル詳細の編集モードで既存値が初期表示されることを確認（登録・保存は実行していないためstg DBに変更なし）。

**2026-09-22 フェーズ5を切り上げてフェーズ6へ移行**（ユーザー決定）。きっかけは「チャンネルページが
まだ無い」という気づき。フェーズ6完了後にフェーズ6.5として自己ユーザーテスト期間を再度設ける。

### フェーズ6: 気づきベースの改善（2026-09-22計画）

フェーズ5で溜まった未解決事項と、上記「未実装機能の棚卸し」を並べ直す。**仕様書の順ではなく、
実際に使ってみて「無くて困った順」**で優先順位を付ける（楽天ブックスAPI連携はフェーズ4.5ステップ8へ前倒し済み）。

**2026-09-22 項目と順序を確定**（ユーザー決定。3案から「一人で使うときに効くものを先に、まとめ・
プロフィールは他人がいて初めて価値が出るため後半に」の案を採用）:

| # | 項目 | 理由 |
|---|---|---|
| 1 | チャンネル（探す→詳細） | 今回の気づき。`channels`は登録時に書き込み済みで着手条件が揃っている |
| 2 | TOP本実装 | 毎日見るページ。各セクションのデータ（mylist/playlists/tags）は既存 |
| 3 | 管理画面の最小版 | 一人で使うときの運営の手間を減らす |
| 4 | まとめ機能 | プロフィールの前提（まとめ仕様書 §9 でプロフィールにまとめセクションを載せる） |
| 5 | プロフィール | まとめセクション込みで一度に作る |

フェーズ7の項目（法務ページ・パスワード再設定・一般ユーザー登録/提案フロー・通報受け皿）は前倒ししない
（ユーザー決定）。ステップ2以降は各ステップ着手時に個別に計画する（これまでのフェーズと同じ運用）。

1. ~~**チャンネル（探す→詳細）**~~（`ページ チャンネル 探す 仕様書` / `ページ チャンネル 詳細 仕様書`）
   → **2026-09-22対応済み**（`wiki/sources/2026-09-22-phase6-plan-and-step1-channels.md`）。
   - `/channels`（`app/(main)/channels/page.tsx`、`ComingSoon`から差し替え）: `/games`と同じ簡易実装方針
     （`channels`全件＋公開`playlists`の集計をクライアント取得、キーワード部分一致・3種ソート・20件ページング）。
     カードは`components/channels/ChannelListCard.tsx`（情報エリア→詳細、ピックアップ各行→プレーヤーの
     個別リンク。`components/playlists/ChannelCard.tsx`（配信者カード）と名前が衝突するため別名）。
     動画数・ピックアップ（スコア上位3件）は`lib/playlist-aggregates.ts`の`fetchChannelAggregates`で
     公開再生リストから集計（`fetchGameVideoCounts`と同型）。モバイルは仕様どおりアコーディオン無しで
     検索欄を常時表示。
   - `/channels/[channelId]`（`app/(main)/channels/[channelId]/page.tsx`、新規）: `channels`を`onSnapshot`購読、
     アイコン（仕様どおり正方形）・名前・再生リスト数/動画数・「再生リストを追加する」・説明文＋AIバッジ。
     再生リスト一覧は`GamePlaylistSection`の一覧UIを`components/playlists/PlaylistListSection.tsx`に切り出して
     共用（`GamePlaylistSection`は取得＋ラップだけの薄い皮に。`fetchPlaylistsByChannel`を`PlaylistGrid.tsx`に追加、
     等価条件のみのため複合インデックス不要）。
   - 説明文の管理者編集: インライン編集（テキストエリア＋保存/キャンセル＋AIバッジ`Checkbox`）＋
     `app/api/admin/channels/[channelId]/route.ts`（PATCH、`requireAdmin`）。**仕様からの緩和**: 説明文未設定の
     チャンネルにも管理者には「説明文を追加する」を出す（後から付ける手段が他に無いため。詳細仕様書 v1.7に注記）。
   - `/playlists/new?channelId=`（追加する仕様書 §3.3・§5.2）: 案内エリア表示＋プレビュー時の流入元チャンネル
     一致確認（NG行＋インライン＋トースト）、`register/route.ts`に`sourceChannelId`を渡して不一致は
     `422 channel_mismatch`で拒否する二重チェック。preview/route.tsの「将来ここに追加」コメントを更新。
   - 「再生リストを追加する」ボタンは`GamePlaylistSection`と同じく全ユーザー共通で登録ページへ遷移
     （一般ユーザー向け「提案する」文言・提案フローはフェーズ7。詳細仕様書 v1.7に注記）。
   - 仕様書: 探す仕様書 v1.3（冒頭に二重に貼られていた§1〜§3.3を整理＋実装注記）、詳細仕様書 v1.7
     （入れ違っていたメタデータ・目次・§1を整形＋実装注記）、追加する仕様書 v3.13（§3.3実装注記）。
   - **動作確認**: `npm run lint` / `npx tsc --noEmit` / `npm run build`成功。ローカル`npm run dev`（stg、
     チャンネル11件）＋Playwrightで、一覧（件数・キーワード絞り込み/0件の空状態/リセット・動画数順ソートと
     同数時の名前順・ピックアップ）、詳細（管理者で説明文の追加→保存→onSnapshot即時反映→編集→キャンセル、
...
3. ~~**管理画面の最小版**~~（`管理_ダッシュボード仕様書`／`管理 審査ワークフロー仕様書`）
   → **2026-09-22対応済み**（`wiki/sources/2026-09-22-phase6-step3-admin-minimal.md`）。
   - **範囲確定**（着手時ユーザー決定）: A（ダッシュボードへの機能集約）＋ B（審査ワークフローのうち
     **通報のみ**の一覧・承認/却下）。マスタ管理・ユーザー管理・お知らせ・お問い合わせ・モニタリング・
     操作ログ・提案系ワークフローはスコープ外。権限はoperator/owner共通のみ（STEP2最終承認等の
     オーナー限定操作は作らない）。
   - `/admin`（新規、`app/admin/page.tsx`）: 通報未処理件数サマリー＋「新着動画を再取得」ボタン
     （`/notifications`下部の「管理者用（暫定）」カードから移設。`refresh-new-videos/route.ts`は変更なし）
     と直近`batch_logs`の実行結果。仕様書の5セクション構成から2カードへ縮小。
   - `/admin/workflows`・`/admin/workflows/[workflowId]`（新規）: `type=='review_report'`の等価条件のみ
     クライアント取得（複合インデックス不要）。STEP1/STEP2の二段階審査・AI運営者・ジャンル別アサインは
     実装せず、単一ステップ（審査中→承認/却下）とした。承認時は`app/api/admin/workflows/[workflowId]/
     route.ts`（PATCH、`requireAdmin`）が対象`reviews`をAdmin SDKで物理削除しスコア再計算
     （`app/api/reviews/upsert/route.ts`のDELETEと同じパターンを再利用）、却下時は`workflows`のみ更新
     （レビューは掲載継続）。運営メモ編集（1,000文字上限）も同APIに集約。
   - `app/admin/layout.tsx`・`components/admin/AdminFrame.tsx`（新規）: 認可ガード（未ログイン→`/login`、
     非管理者→`/`）。「共通UIコンポーネント（管理画面）仕様書」が存在しないため3ペイン構造ではなく
     簡易ヘッダー（ロゴ相当＋ナビ2項目＋ユーザーメニュー、モバイルは2段目に折り返し）で代替。
   - ログイン後リダイレクトをロール別に分岐（管理者→`/admin`、一般→`/`。§5.4の据え置きを解消）、
     `UserDropdown`に管理者のみの「管理画面」リンクを追加。
   - **dev-orchestratorレビュー**（spec-conformance-reviewer・design-consistency-reviewer・
     phase-finish-checkerを並列委任）で3件対応: ①スコープ縮小がユーザー承認済みにもかかわらず対応する
     2仕様書に一切反映されていなかったため実装注記節を追加（管理_ダッシュボード仕様書v1.5・管理 審査
     ワークフロー仕様書v8.0）、あわせて古い`/admin/login`表記をページ一覧仕様書v1.7で`/login`に修正、
     ②ダッシュボードの件数表示が`text-3xl`（デザイントークン仕様書のスケール外）だったため`text-score`
     に修正、③`AdminFrame`のヘッダーナビにモバイル幅の折り返し対策が無かったため2段目レイアウトに変更。
   - **動作確認**: `npm run lint` / `npx tsc --noEmit` / `npm run build`成功（レビュー対応前後の両方）。
     ローカル`npm run dev`（stg）＋Playwrightで、未ログイン/一般ユーザーの`/admin`アクセス制御、
     管理者ログイン→`/admin`自動遷移、テスト用通報の作成→一覧（タブ・件数）→詳細→承認（対象レビュー
     削除・スコア再計算を確認）→別途作成した通報の却下（理由必須バリデーション・掲載継続を確認）、
     運営メモ編集を確認。1440/768/390px幅で横スクロール無し。確認用に作成した`workflows`・`reviews`・
     `mylist`のデータはすべてstgから削除し元の状態に復元した。
4. **まとめ機能**（`ページ 再生リスト まとめ機能仕様書`）— 着手時に計画。
5. **プロフィール**（`ページ 設定 プロフィール仕様書`＋まとめ仕様書 §9）— 着手時に計画。

### フェーズ6.5: 自己ユーザーテスト期間（再）

フェーズ6完了後に実施。運用はフェーズ5と同じ（記録フォーマットなし、その場で直す、直せないものは
未解決事項に1行）。
