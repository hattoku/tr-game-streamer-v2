...
3. ~~**タグシステム**~~（`ページ ゲームタイトル 詳細仕様書.md`第8章 /
   ...
   - **ブラウザでの動作確認は未実施**（実装・ビルド確認のみ）。次セッションでの確認推奨。
4. **「再生リストを探す」本実装**（`ページ 再生リストを探す 仕様書.md`）→ 未着手（次にやること）。
   `app/(main)/playlists/page.tsx`の最小版を2カラム構成に差し替える。ステップ2で`/games`用に
   作った検索条件パネル・カードグリッドのレイアウトパターンを共通化して再利用することを検討
   （例: `components/search/SearchPanel.tsx`）。ソートのデフォルト「スコアが高い順」はステップ1で
   スコアが実際に入るようになったため対応可能。
5. **仕上げ** → 未着手。フェーズ2.5ステップ8と同内容（3幅目視確認・キーボード操作/aria・
   `npm run lint`/`tsc`/`build`・本番E2E確認・本HANDOFF更新とWiki記録）を、ステップ1〜4の
   全機能に対して行う。
