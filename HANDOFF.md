...
4. ~~**「再生リストを探す」本実装**~~（`ページ 再生リストを探す 仕様書.md`）→ **2026-09-15対応済み**。
   ...
   （`scripts/dev/screenshot.mjs`使用）。タグ絞り込みは本番に`playlistTagIds`を持つ再生リストが
   まだ無いため、パネル自体の表示は未確認（`resolveTags`/`fetchTagsMap`は`/games`・タグ編集モーダルで
   確認済みの共通コードを再利用しているため実装上のリスクは低いと判断）。
