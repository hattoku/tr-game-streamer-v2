...
3. マスタデータ投入基盤の整備
   - `firebase-admin`導入・サービスアカウント運用方針の決定（`SECRET_MANAGEMENT.md`と整合させる）。
     `firestore.rules`で`genres`/`tags`等マスタ系は`write: if isAdmin()`のみ許可のため、
     認証未実装の現段階ではAdmin SDK（ルールをバイパス）での投入が必須。
   - `tags`コレクション用の連番ID採番ロジック（`TAG-{連番}`形式、1始まり）をトランザクションで実装。
     `document/specification/management/管理 マスタ管理仕様書.md`6.2/6.3節参照。ゲームタイトルタグ・
     再生リストタグは同一`tags`コレクションを共有するため採番も共通。運用開始後に管理画面/ユーザーが
     追加するタグと連番が衝突しない設計にすること。ジャンル・テーマには採番規約の記載がなくFirestore
     自動IDでよい。
4. マスタデータ投入実行（`document/master/`配下の初期データをFirestoreへ）
   - 対象はジャンル9件・テーマ約50件・タグ約109件（ゲームタイトルタグ56+再生リストタグ53）の
     合計約170件。実ゲームタイトル（`games`コレクション）は楽天ブックスAPI取得＋将来のAI運営者
     新作検知バッチで賄う設計のため、フェーズ1の手動投入対象には含まれない
     （`document/specification/db/Firestore データモデル設計書.md`3.5節参照）。
   - 投入前に、本番のみ先行投入するかステージング（`tr-game-streamer-stg`、下記5.）にも同時投入するかを
     決めておく。決めずに進めると本番/ステージングのマスタ不一致が発生する。
