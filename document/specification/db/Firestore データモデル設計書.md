...
| `hasNewArrival` | boolean | ✅ | 新着動画フラグ（動画更新バッチで更新、ユーザーが確認で `false` に戻す） |
...
YouTube定期取得バッチ（動画更新バッチ / チャンネル更新バッチ）の実行結果ログ。
...
| `newVideoCount` | number? | — | 新規検出した動画数（動画更新バッチのみ） |
...
動画更新バッチ / チャンネル更新バッチ によるYouTube APIクォータ消費の記録。管理画面のモニタリング表示に使用。
...
AI運営者（コンテンツ収集バッチ）が収集した再生リスト候補。オーナーのレビュー待ちキュー。
...
コンテンツ収集バッチ（AI運営者コンテンツ収集バッチ）の実行結果ログ。
...
コンテンツ収集バッチ によるAPIクォータ消費記録。既存の `api_quota_logs` とは分離管理。
...
AI運営者がコンテンツ収集バッチで使用するジャンル別の検索キーワードリスト。
...
AI運営者（審査AI）の設定情報。`users` コレクションと対になる形で管理する。
...
| `displayName` | string | ✅ | AI運営者表示名（例：`"RPG審査AI"`） |
...
| `genres` | 物理削除 | `games.genreId` を外す。対応AI運営者は保持 |
...
| v1.2 | 2026-03-24 | プロジェクト全体のバッチ名称統一に伴い、Function A〜C の表記を正式名称（動画更新/チャンネル更新/コンテンツ収集）に更新。ログ定義内の識別子例も修正。 |